...
class order extends database
{
    ...
    {
        $sql =
            'select*from tbl_pesan,tbl_user,tbl_daerah,tbl_paket,tbl_hotel where tbl_pesan.id_user=tbl_user.id_user and tbl_pesan.id_paket=tbl_paket.id_paket and tbl_pesan.id_hotel=tbl_hotel.id_hotel and tbl_pesan.id_daerah=tbl_daerah.id_daerah ';
        $result = mysqli_query($this->getConnection(), $sql);
    ...
    {
        $sql =
            'select tbl_user.nama_user,tbl_pesan.id_pesan,tbl_bukti.file from tbl_pesan,tbl_user,tbl_bukti where tbl_pesan.id_user=tbl_user.id_user and tbl_pesan.id_pesan=tbl_bukti.id_pesan ';
        $result = mysqli_query($this->getConnection(), $sql);
        while ($row = mysqli_fetch_array($result)) {
            $hasil[] = $row;
        }
    ...
    {
        $sql =
            "select count(status)as status from tbl_pesan where status='S2' ";
        $result = mysqli_query($this->getConnection(), $sql);
        while ($row = mysqli_fetch_array($result)) {
            $hasil[] = $row;
        }
